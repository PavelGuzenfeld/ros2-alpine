...
def test_opencv():
    ...
    try:
        ...
        gst_support = cv2.getBuildInformation().find("GStreamer") != -1
        print_result("GStreamer Integration", gst_support)
...
def test_integration():
    ...
    try:
        # Test OpenCV + GStreamer pipeline
        import cv2
        ...
        if cap.isOpened():
            ret, frame = cap.read()
            cap.release()
            print_result("OpenCV + GStreamer", ret and frame is not None, f"Frame shape: {frame.shape if ret else 'None'}")
        else:
            print_result("OpenCV + GStreamer", False, "Failed to open GStreamer pipeline")
